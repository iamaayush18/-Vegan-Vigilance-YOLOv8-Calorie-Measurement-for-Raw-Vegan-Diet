...
def estimate_calories():
    ...
    #foodnum = len(ans)
    food_list_str = request.form['food_list']
    food_list = [f.strip() for f in food_list_str.split(",")]
    ...
    for food in ans:
        ...
        my_int=int(my_float)
        total_calories +=float(my_int * float(food_list[0]))
        food_list.pop(0)
    
    # Return the total calorie intake as a JSON response
    #response = {
    #'labels': labels,
    #'total_calories': f"{total_calories} cal"
#}
    response = {
    'labels': labels,
    'total_calories': f"{total_calories} cal"
